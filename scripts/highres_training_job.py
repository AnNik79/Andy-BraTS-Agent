"""Supervise a High-Resolution expert training run and record terminal failures."""
import argparse
import os
...
def main():
    ...
    if args.init_weights:
        raise ValueError("HighRes starts from fresh initialization; refusing --init-weights")
    ...
    try:
        ...
        destination = train_expert("highres", records, splits, cfg,
                                   resume=args.resume, init_weights=None)
        marker = Path(destination).parent / "RUN_COMPLETE"
        marker.write_text(f"{destination}\n")
        completed = True
    except BaseException as exc:
        save_json(Path(cfg["output_dir"]) / "checkpoints" / "highres" / "status.json",
